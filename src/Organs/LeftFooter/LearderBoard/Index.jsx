import { Card, Table, Text, Divider, SegmentedControl, Center, Box } from "@mantine/core";
import styled from "styled-components";
import { leaderBoardDummyData, leaderBoardDummyDataGlobal } from "../../../Data/Dummies/LeaderBoard";
...
const rowsWeekly = leaderBoardDummyData.map((element) => (
  <tr key={element.name}>
    ...
    <td style={{height:"22px",width:"100%",textAlign:"end"}}>{element.username}</td>
  </tr>
));
...
function LeaderBoardIndex() {
  ...
  const [currentColumnNames,setCurrentColumnNames] = useState(columnNamesWeekly)
  ...
  return (
    <SLeaderBoardIndex>
       ...
       <Divider my="sm" color={"black"} style={{ marginTop:"5px" }}/>
        <Card shadow="sm" p="lg" radius="md" withBorder style={{paddingTop:"8px",paddingBottom:"8px", display:"flex",flexDirection:"column",alignItems:"center"}}>
            <SegmentedControl 
                value={currentBoard}
                onChange={(e)=>{
                      setCurrentBoard(e)
                ...
                data={[
                  { label: (
                    <Center>
                        <Box ml={5} mr={5}>Weekly</Box>
                    </Center>
                  ) , value: 'weekly' },
                  { label: (
                    <Center>
                        <Box ml={5} mr={5}>Global</Box>
                    </Center>
                  ), value: 'global' },
                ]}
            />
